...
func main() {
	...
	interestingSmolCave := ""
	pathOptions = findPath([]string{"start"}, pathOptions, interestingSmolCave)
...
func findPath(path []string, pathOptions [][]string, interestingSmolCave string) [][]string {
	...
	for _, connectedCave := range caveConnections[startPoint] {
		if connectedCave == "end" {
			...
			pathOptions = findPath(localPathCopy(path, connectedCave), pathOptions, interestingSmolCave)
		} else {
			if connectedCave == "start" {
				continue
			} else if !contains(localPathCopy(path, ""), connectedCave) {
				pathOptions = findPath(localPathCopy(path, connectedCave), pathOptions, interestingSmolCave)
			} else if interestingSmolCave == "" {
				pathOptions = findPath(localPathCopy(path, connectedCave), pathOptions, connectedCave)
			}
		}
	}
	return pathOptions
